...
export default function App() {
  ...
  // MULTI-AGENT LOGIC: Any email containing 'delivery' or 'agent' becomes Delivery Boy
  ...
  const sendEmailAlert = async (subject: string, message: string, customerEmail: string = 'admin@nectarroots.com') => {
    ...
    try {
      await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({ access_key: myAccessKey, subject, from_name: 'Nectar Roots', email: customerEmail, message }),
      });
    } catch (error) {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    else { showToast('Secure Dashboard Accessed 👑'); closeModal(); }
  };
  ...
  const handleCustomerSignup = async (e: React.FormEvent) => {
    ...
    if (error) setLoginError(error.message);
      ...
      const newCustomer = {
        ...
        wallet_balance: 0, qrCode: `NR-${Math.floor(1000 + Math.random() * 9000)}`,
      };
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithPassword({ email: emailInput, password: passwordInput });
    if (error) setLoginError(error.message);
    else { showToast('Logged in successfully! 🔓'); closeModal(); }
  };
  ...
  const handleForgotPassword = async () => {
    ...
    if (error) setLoginError(error.message);
    else { showToast('Reset link sent! 📧'); setLoginError(''); }
  };

  const handleLogout = async () => {
    await supabase.auth.signOut(); setRole('guest'); showToast('Securely logged out 👋');
  };
  ...
  const handleCreateSubscription = async (e: React.FormEvent) => {
    ...
    try {
      const newSub = {
        customer_id: currentCustomer.id, customer_name: currentCustomer.name, product_name: selectedSubProduct.name,
        quantity: subQty, price: selectedSubProduct.price * subQty, frequency: subFreq, payment_type: subPayType, status: 'Active'
      };
  ...
  // DELIVERY BOY LOGIC (Agent Tracking Included)
    ...
    try {
      if (delivery.payment_type === 'scan_deduct') {
        if (Number(cust.wallet_balance) < amountToDeduct) {
          return showToast(`Failed: Low balance! (₹${cust.wallet_balance})`, 'error');
        }
        const newBal = Number(cust.wallet_balance) - amountToDeduct;
        await supabase.from('customers').update({ wallet_balance: newBal }).eq('id', cust.id);

        // Appending Agent ID to track history
        await supabase.from('transactions').insert([{
          customer_name: cust.name,
          item: `Sub Delivery: ${delivery.product_name} (x${delivery.quantity}) [Agent: ${user.email}]`,
        ...
        await supabase.from('transactions').insert([{
          customer_name: cust.name,
          item: `Sub Delivery (Auto-paid): ${delivery.product_name} (x${delivery.quantity}) [Agent: ${user.email}]`,
          amount: 0
        }]);
      }
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.from('products').insert([{
        name: newProd.name, category: newProd.category, price: Number(newProd.price), original_price: Number(newProd.price) + 15, unit: newProd.unit, icon: '🌿', tag: newProd.tag,
    }]).select();
    if (data) {
  ...
  const handleUpdateProduct = async (e: React.FormEvent) => {
    ...
    setEditingProduct(null); showToast('Product updated!');
  };

  const handleDeleteProduct = async (id: string) => {
    if (!window.confirm('Delete this product?')) return;
    await supabase.from('products').delete().eq('id', id);
    setProducts(products.filter((p) => String(p.id) !== String(id))); showToast('Product deleted!');
  };

  const handleAddToCart = (product: any) => {
    const existingIndex = cart.findIndex((item) => String(item.id) === String(product.id));
    if (existingIndex > -1) {
      const updatedCart = [...cart]; updatedCart[existingIndex].quantity += 1; setCart(updatedCart);
  ...
  const handleUpdateCartQuantity = (id: any, delta: number) => {
    setCart(cart.map((item) => String(item.id) === String(id) ? { ...item, quantity: item.quantity + delta > 0 ? item.quantity + delta : 0 } : item).filter(i => i.quantity > 0) as any);
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const newTxs = cart.map((item) => ({ customer_name: activeCustomer.name, item: `${item.name} (${item.quantity} ${item.unit})`, amount: item.price * item.quantity }));
  ...
  const handleRecharge = async (custId: string) => {
    ...
    if (!cust) return;
    const newBal = Number(cust.wallet_balance || 0) + 500;
    try {
      await supabase.from('customers').update({ wallet_balance: newBal }).eq('id', custId);
      await supabase.from('transactions').insert([{ customer_name: cust.name, item: `Wallet Recharge (+₹500)`, amount: 500 }]);
    ...
    <div className="min-h-screen bg-[#F8F5EE] text-[#2D241E] font-sans pb-28 antialiased selection:bg-[#EBE5D9] relative">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {(!user || role === 'guest') && (
            <button onClick={() => setShowLoginModal(true)} className="bg-[#F8F5EE]/10 hover:bg-[#F8F5EE]/20 border border-[#F8F5EE]/20 text-[#F4F0E6] font-semibold px-2 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-xs transition">
              <span className="hidden sm:inline">Login / Signup</span><span className="sm:inline block">Login</span>
            </button>
          )}

          {user && role === 'customer' && (
            <button onClick={() => showToast('Wallet balances update automatically.')} className="bg-[#2C523D] hover:bg-[#3A6B50] border border-[#3A6B50] text-[#F4F0E6] px-2 py-1.5 rounded-xl text-[11px] sm:text-xs font-bold flex items-center gap-1 shadow-inner transition">
              <span>💳</span><span>₹{currentCustomer?.wallet_balance || 0}</span>
            </button>
          )}

          {user && (role === 'admin' || role === 'delivery') && (
            <button onClick={handleLogout} className="bg-[#8B0000] hover:bg-[#5C0000] text-white font-bold px-2 sm:px-3 py-1.5 rounded-xl text-[11px] sm:text-xs transition border border-[#5C0000]">Exit</button>
          )}
        </div>
      </header>
      ...
      {role === 'admin' && (
        <div className="max-w-4xl mx-auto p-3.5 sm:p-5 space-y-4">
          {/* Admin content remains same as previous... */}
          <div className="bg-gradient-to-br from-[#1E3F2D] to-[#2C523D] text-[#F4F0E6] p-4 sm:p-5 rounded-3xl shadow-lg border border-[#152E20] flex justify-between items-center">
            <div><span className="bg-[#B5651D]/20 text-[#D79A5E] border border-[#B5651D]/40 text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase mb-1 inline-block">Master Dashboard</span><h1 className="font-extrabold text-base sm:text-lg text-white">Nectar Roots Control Panel</h1></div>
          ...
          <div className="bg-white p-1.5 rounded-2xl border border-[#EBE5D9] shadow-sm grid grid-cols-2 sm:grid-cols-4 gap-1.5">
            {['customers', 'products', 'finance', 'delivery'].map((tab) => (
              <button key={tab} onClick={() => setAdminTab(tab as any)} className={`py-2 px-3 rounded-xl text-xs transition-all flex items-center justify-center gap-1.5 ${adminTab === tab ? 'bg-[#1E3F2D] text-[#F4F0E6] font-bold shadow-md' : 'text-[#796C61] hover:bg-[#F0EBE1] font-semibold'} capitalize`}><span>{tab === 'customers' ? '👥' : tab === 'products' ? '📦' : tab === 'finance' ? '📊' : '🛵'}</span> {tab}</button>
            ))}
          </div>
          ...
          {adminTab === 'customers' && (
            <div className="bg-white p-4 rounded-3xl border border-[#EBE5D9] shadow-sm">
              ...
              {customers.map((c) => (
                <div key={c.id} className="p-4 bg-[#F8F5EE] rounded-2xl border border-[#EBE5D9] mb-3 flex justify-between items-center">
                  <div><div className="font-extrabold text-xs text-[#2D241E]">{c.name}</div><div className="text-[11px] text-[#796C61] mt-1">📞 {c.phone} | ✉️ {c.email}</div><div className="text-[11px] font-extrabold text-[#B5651D] mt-1">Wallet: ₹{c.wallet_balance || 0}</div></div>
                  <button onClick={() => handleRecharge(c.id)} className="bg-[#1E3F2D] hover:bg-[#152E20] text-[#F4F0E6] text-xs px-4 py-2 rounded-xl font-bold">+ ₹500</button>
                </div>
              ))}
            </div>
          )}
          ...
          {adminTab === 'products' && (
            <div className="space-y-4">
              ...
              <div className="bg-white p-4 rounded-3xl border border-[#EBE5D9] shadow-sm">
                {products.map((p) => (
                  <div key={p.id} className="py-3 flex justify-between items-center border-b border-[#EBE5D9] last:border-0"><div className="font-bold text-xs">{p.name} <span className="text-[#B5651D]">₹{p.price}</span></div><button onClick={() => handleDeleteProduct(p.id)} className="text-[#8B0000] text-[11px] font-bold">Delete</button></div>
                ))}
              </div>
            </div>
          )}
          ...
          {adminTab === 'finance' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white p-4 rounded-3xl border border-[#EBE5D9] shadow-sm"><div className="text-[10px] font-bold text-[#796C61] uppercase tracking-wide">Total Sales</div><div className="text-xl font-extrabold text-[#2D241E] mt-1">₹{totalSales}</div></div>
                <div className="bg-[#1E3F2D] p-4 rounded-3xl border border-[#152E20] shadow-md text-[#F4F0E6]"><div className="text-[10px] font-bold text-[#A5C0A0] uppercase tracking-wide">Wallet Recharges</div><div className="text-xl font-extrabold text-white mt-1">₹{totalRecharges}</div></div>
              </div>
            </div>
          )}
        </div>
      ...
      {/* 🛵 DELIVERY BOY DASHBOARD (MULTI-AGENT & HISTORY) */}
      {role === 'delivery' && (
        <div className="max-w-md mx-auto p-3.5 sm:p-5 space-y-4">
          <div className="bg-gradient-to-br from-[#B5651D] to-[#965216] text-[#F4F0E6] p-4 sm:p-5 rounded-3xl shadow-lg border border-[#965216] flex justify-between items-center">
            <div>
              <span className="bg-white/20 text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider mb-1 inline-block">Agent ID: {user?.email?.split('@')[0]}</span>
              <h1 className="font-extrabold text-base sm:text-lg text-white">Delivery Portal</h1>
            </div>
                ...
                return (
                  <div key={idx} className="bg-white p-4 rounded-3xl border border-[#EBE5D9] shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-center bg-[#F8F5EE] p-2.5 rounded-xl mb-3 border border-[#EBE5D9]">
                      <div className="text-xs font-bold text-[#1E3F2D] flex items-center gap-1.5"><span>🥛</span> {sub.product_name}</div>
                      <div className="text-xs font-extrabold text-[#B5651D] bg-white px-2 py-0.5 rounded-md border border-[#EBE5D9]">{sub.quantity} {sub.unit || 'qty'}</div>
                    </div>
                    
                    <div className="flex justify-between items-center mt-2">
                      <div className="text-[10px] font-bold text-[#796C61]">{sub.payment_type === 'scan_deduct' ? '📱 Collect via QR Scan' : '⚡ Auto-Paid (Wallet)'}</div>
                      <button onClick={() => { setSelectedDelivery({...sub, cust}); setShowScannerModal(true); }} className="bg-[#1E3F2D] hover:bg-[#152E20] text-white px-4 py-2 rounded-xl text-xs font-bold transition shadow-sm active:scale-95">Scan & Deliver</button>
                    </div>
                  </div>
                );
              })}
          ...
          {/* TAB 2: AGENT HISTORY & EARNINGS */}
          {deliveryTab === 'history' && (
            <div className="space-y-4">
              <div className="bg-white p-4 rounded-3xl border border-[#EBE5D9] shadow-sm flex justify-between items-center">
                <div>
                  <div className="text-[10px] font-bold text-[#796C61] uppercase tracking-wide">My Total Collection</div>
                  <div className="text-[10px] text-[#B5651D] font-medium">(From QR Scans)</div>
                </div>
                <div className="text-xl font-extrabold text-[#1E3F2D]">₹{agentTotalCollected}</div>
              </div>

              <div className="bg-white p-4 rounded-3xl border border-[#EBE5D9] shadow-sm">
                <h3 className="font-bold text-sm text-[#2D241E] mb-3">📜 My Delivery Logs</h3>
                {agentTransactions.length === 0 && <div className="text-xs text-center py-6 text-[#796C61] font-medium">You haven't made any deliveries yet.</div>}
                
                <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                  {agentTransactions.map((t, idx) => {
                    const cleanItemName = t.item?.split(' [Agent:')[0]; // Hide agent email from UI
                    return (
                      <div key={idx} className="p-3 bg-[#F8F5EE] rounded-2xl border border-[#EBE5D9] flex justify-between items-center gap-3">
                        <div>
                          <div className="font-bold text-[#2D241E] text-xs leading-snug line-clamp-2">{cleanItemName}</div>
                          <div className="text-[10px] text-[#796C61] font-medium mt-0.5">To: <span className="font-bold">{t.customer_name}</span></div>
                          <div className="text-[9px] text-[#B5651D] font-bold mt-0.5">{new Date(t.created_at).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}</div>
                        </div>
                        <div className="font-extrabold text-[#1E3F2D] text-xs shrink-0 bg-white border border-[#EBE5D9] px-2 py-1 rounded-lg">₹{t.amount}</div>
                      </div>
                    )
                  })}
                </div>
      ...
      {/* STOREFRONT & MODALS (Kept exactly same for customers) */}
      {(role === 'guest' || role === 'customer') && (
                ...
                <div key={p.id} className="bg-white p-4 rounded-3xl shadow-sm border border-[#EBE5D9] flex flex-col justify-between hover:shadow-md transition-all group">
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    {qtyInCart > 0 ? (
                      ...
                      <button onClick={() => handleAddToCart(p)} className="w-full bg-[#1E3F2D] hover:bg-[#152E20] text-[#F4F0E6] font-bold py-2 rounded-xl text-xs shadow-md transition active:scale-95">🛒 Add</button>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="bg-[#F8F5EE] rounded-t-3xl sm:rounded-3xl p-5 max-w-sm w-full shadow-2xl flex flex-col max-h-[85vh] animate-slide-up border border-[#EBE5D9]">
            ...
            <div className="overflow-y-auto pr-1 space-y-5 pb-2">
              ...
              <form onSubmit={handleCreateSubscription} className="space-y-5">
                <div><label className="text-[11px] font-bold text-[#796C61] uppercase tracking-wide block mb-2">Quantity per day</label><div className="flex items-center gap-2">{[1, 2, 3, 5].map((q) => (<button key={q} type="button" onClick={() => setSubQty(q)} className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold transition-all border ${subQty === q ? 'bg-[#1E3F2D] text-[#F4F0E6] border-[#1E3F2D] shadow-md' : 'bg-white text-[#2D241E] border-[#EBE5D9] hover:bg-[#F0EBE1]'}`}>{q}</button>))}</div></div>
                <div><label className="text-[11px] font-bold text-[#796C61] uppercase tracking-wide block mb-2">Frequency</label><div className="flex gap-2"><button type="button" onClick={() => setSubFreq('Daily')} className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all border ${subFreq === 'Daily' ? 'bg-[#1E3F2D] text-[#F4F0E6] border-[#1E3F2D]' : 'bg-white text-[#2D241E] border-[#EBE5D9]'}`}>📅 Everyday</button><button type="button" onClick={() => setSubFreq('Alternate Days')} className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all border ${subFreq === 'Alternate Days' ? 'bg-[#1E3F2D] text-[#F4F0E6] border-[#1E3F2D]' : 'bg-white text-[#2D241E] border-[#EBE5D9]'}`}>🗓️ Alt. Days</button></div></div>
                <div><label className="text-[11px] font-bold text-[#796C61] uppercase tracking-wide block mb-2">Payment Preference</label><div className="space-y-2">
                  <label className={`p-3.5 rounded-2xl border flex items-start gap-3 cursor-pointer ${subPayType === 'scan_deduct' ? 'bg-[#F0EBE1] border-[#1E3F2D]' : 'bg-white border-[#EBE5D9]'}`}><input type="radio" name="payType" checked={subPayType === 'scan_deduct'} onChange={() => setSubPayType('scan_deduct')} className="mt-0.5 accent-[#1E3F2D] w-4 h-4" /><div><div className="text-xs font-bold text-[#2D241E]">📱 Cut Money After QR Scan</div></div></label>
                  <label className={`p-3.5 rounded-2xl border flex items-start gap-3 cursor-pointer ${subPayType === 'auto_deduct' ? 'bg-[#F0EBE1] border-[#1E3F2D]' : 'bg-white border-[#EBE5D9]'}`}><input type="radio" name="payType" checked={subPayType === 'auto_deduct'} onChange={() => setSubPayType('auto_deduct')} className="mt-0.5 accent-[#1E3F2D] w-4 h-4" /><div><div className="text-xs font-bold text-[#2D241E]">⚡ Auto-Deduct Morning</div></div></label>
                </div></div>
                <div className="pt-2 sticky bottom-0 bg-[#F8F5EE] pb-1"><button type="submit" className="w-full bg-[#1E3F2D] text-[#F4F0E6] font-extrabold py-3.5 rounded-xl text-xs shadow-lg active:scale-95">Confirm (₹{selectedSubProduct.price * subQty} / day) ➔</button></div>
              </form>
            </div>
          </div>
        </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-3.5 z-50">
          <div className="bg-[#F8F5EE] rounded-3xl p-5 max-w-sm w-full shadow-2xl border border-[#EBE5D9]">
            <div className="flex gap-1 mb-5 bg-white p-1.5 rounded-2xl border border-[#EBE5D9] shadow-sm"><button onClick={() => setAuthRoleTab('customer')} className={`w-1/2 py-2 text-xs font-bold rounded-xl transition-all ${authRoleTab === 'customer' ? 'bg-[#1E3F2D] text-[#F4F0E6]' : 'text-[#796C61]'}`}>👤 Customer</button><button onClick={() => setAuthRoleTab('admin')} className={`w-1/2 py-2 text-xs font-bold rounded-xl transition-all ${authRoleTab === 'admin' ? 'bg-[#1E3F2D] text-[#F4F0E6]' : 'text-[#796C61]'}`}>🛡️ Staff/Admin</button></div>
            {loginError && <div className="mb-4 text-[10px] text-[#8B0000] bg-[#8B0000]/10 p-2.5 rounded-xl font-medium">{loginError}</div>}
            {authRoleTab === 'admin' ? (
              <form onSubmit={handleAdminLogin} className="space-y-3.5"><input type="email" placeholder="Staff/Admin Email" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9]" required /><input type="password" placeholder="Password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9]" required />
              <p className="text-[10px] text-center text-[#796C61] font-medium mt-1">Use admin@... or any delivery@... email</p>
              <button type="submit" className="w-full bg-[#1E3F2D] text-[#F4F0E6] text-xs py-3.5 rounded-xl font-extrabold shadow-md mt-2">Login Securely ➔</button></form>
            ) : (
              <form className="space-y-3.5">
                {authView === 'signup' && (<><input type="text" placeholder="Full Name" value={signupName} onChange={(e) => setSignupName(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9]" required /><input type="tel" placeholder="Mobile Number" value={signupPhone} onChange={(e) => setSignupPhone(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9]" required /><textarea placeholder="Delivery Address" value={signupAddress} onChange={(e) => setSignupAddress(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9] h-16" required /></>)}
                <input type="email" placeholder="Email Address" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9]" required /><input type="password" placeholder="Password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full text-xs p-3 bg-white rounded-xl border border-[#EBE5D9]" required />
                {authView === 'login' && (<div className="flex justify-end pt-1"><button type="button" onClick={handleForgotPassword} className="text-[10px] text-[#B5651D] font-bold hover:underline">Forgot Password?</button></div>)}
                <button type="button" onClick={authView === 'login' ? handlePasswordLogin : handleCustomerSignup} className="w-full bg-[#1E3F2D] text-[#F4F0E6] text-xs py-3.5 rounded-xl font-extrabold mt-2 shadow-md">{authView === 'login' ? 'Login ➔' : 'Sign Up ➔'}</button>
                <div className="text-center mt-4 text-[11px] text-[#796C61] font-medium">{authView === 'login' ? <>New? <button type="button" onClick={() => setAuthView('signup')} className="text-[#B5651D] font-bold ml-1">Sign Up</button></> : <>Existing? <button type="button" onClick={() => setAuthView('login')} className="text-[#B5651D] font-bold ml-1">Login</button></>}</div>
              </form>
            )}
      ...
      {showScannerModal && selectedDelivery && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-3.5 z-50">
          <div className="bg-[#F8F5EE] rounded-3xl p-5 max-w-sm w-full shadow-2xl border border-[#EBE5D9] text-center">
            ...
            <p className="text-[10px] text-[#796C61] font-bold mb-4">For: {selectedDelivery.cust?.name}</p>
            <div className="w-48 h-48 mx-auto bg-black rounded-2xl border-4 border-[#1E3F2D] border-dashed flex flex-col items-center justify-center mb-4 relative overflow-hidden shadow-inner"><div className="w-full h-1 bg-red-500/60 absolute top-1/2 animate-pulse shadow-[0_0_10px_red]"></div><span className="text-white text-xs opacity-60 font-mono tracking-widest">CAMERA ACTIVE</span></div>
            <div className="bg-white p-3 rounded-xl border border-[#EBE5D9] mb-4 text-left"><div className="text-[10px] text-[#796C61] font-bold uppercase tracking-wide">Deduction Details</div><div className="flex justify-between mt-1"><span className="text-xs font-bold text-[#2D241E]">Wallet:</span><span className="text-xs font-extrabold text-[#1E3F2D]">₹{selectedDelivery.cust?.wallet_balance}</span></div><div className="flex justify-between mt-1"><span className="text-xs font-bold text-[#2D241E]">To Deduct:</span><span className="text-xs font-extrabold text-[#8B0000]">-₹{selectedDelivery.price}</span></div></div>
            <button onClick={() => handleMarkDelivered(selectedDelivery)} className="w-full bg-[#1E3F2D] text-white py-3.5 rounded-xl text-xs font-extrabold shadow-md active:scale-95">Simulate Successful Scan ✅</button>
      ...
      {/* OTHER MODALS (CART, ORDERS, QR)... Keeping same for brevity */}
      {showCartModal && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-3.5 z-50">
      ...
      {showQRModal && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-3.5 z-50">
          <div className="bg-[#F8F5EE] rounded-3xl p-6 max-w-xs w-full text-center space-y-4 shadow-2xl border border-[#EBE5D9]">
            ...
            <button onClick={() => setShowQRModal(false)} className="w-full bg-white border border-[#EBE5D9] hover:bg-[#F0EBE1] text-[#2D241E] text-xs font-bold py-3 rounded-xl transition mt-2">Close</button>
          </div>
        </div>
      )}
      ...
      {showOrdersModal && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-3.5 z-50">
          <div className="bg-[#F8F5EE] rounded-3xl p-5 max-w-sm w-full shadow-2xl space-y-4 border border-[#EBE5D9]">
            <div className="flex justify-between items-center pb-3 border-b border-[#EBE5D9]"><h3 className="font-extrabold text-sm text-[#2D241E]">📦 My Orders & Subs</h3><button onClick={() => setShowOrdersModal(false)} className="text-[#796C61] hover:text-[#2D241E] font-bold bg-white w-8 h-8 rounded-full border border-[#EBE5D9]">✕</button></div>
            <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-1">
              <div>
                ...
                {subscriptions.filter(s => s.customer_id === currentCustomer?.id).map((s, idx) => (
                  <div key={idx} className="p-3.5 bg-white rounded-2xl border border-[#EBE5D9] text-xs shadow-sm"><div className="font-extrabold text-[#1E3F2D]">{s.product_name} <span className="text-[#B5651D]">({s.quantity} qty)</span></div><div className="text-[10px] text-[#796C61] mt-1.5 flex justify-between"><span>{s.payment_type === 'scan_deduct' ? '📱 Scan' : '⚡ Auto'}</span><span className="font-bold">{s.frequency}</span></div></div>
                ))}
              </div>
              <div className="pt-2 border-t border-[#EBE5D9]">
                ...
                {transactions.filter(t => t.customer_name === currentCustomer?.name && !t.item?.includes('Recharge')).map((t, idx) => (
                  <div key={idx} className="p-3 bg-white rounded-2xl border border-[#EBE5D9] text-xs flex justify-between items-center gap-3">
                    <div><div className="font-bold text-[#2D241E] leading-snug line-clamp-2">{t.item.split(' [Agent:')[0]}</div><div className="text-[10px] text-[#796C61]">Paid via Wallet</div></div><div className="font-extrabold text-[#1E3F2D]">₹{t.amount}</div>
                  </div>
                ))}
              </div>
            </div>
