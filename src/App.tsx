...
export default function App() {
  ...
  const [authStep, setAuthStep] = useState<'form' | 'otp_verification'>('form');
  ...
  const fetchLiveDatabaseData = async () => {
    try {
      ...
      if (custData) {
        ...
        if (custData.length > 0 && !selectedCustId)
          setSelectedCustId(String(custData[0].id));
      }
  ...
  // FIX: Checkout logic simplified to rely only on 'user' session
  ...
  const handleDeductDelivery = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const totalCost = Number(prod.price) * Number(delivQty);
    const balance = cust.walletBalance ?? cust.wallet_balance ?? 0;

    if (balance < totalCost) {
      showToast(`Insufficient Balance! Required: ₹${totalCost}`, 'error');
      return;
    }
    ...
    const newTx = {
      ...
      item: `${prod.name} (${delivQty} ${prod.unit})`,
      amount: totalCost,
    };
    ...
    try {
      ...
      fetchLiveDatabaseData();
      showToast(`₹${totalCost} deducted from ${cust.name}`);
    } catch (e: any) {
      alert('Delivery deduction failed: ' + e.message);
    }
  };
  ...
  const handleRecharge = async (custId: string) => {
    ...
    if (!cust) return;
    const newBal = Number(cust.wallet_balance || 0) + Number(rechargeAmt);
    ...
    try {
      ...
      const { error: txErr } = await supabase.from('transactions').insert([
        {
          customer_name: cust.name,
          item: `Wallet Recharge (+₹${rechargeAmt})`,
      ...
      fetchLiveDatabaseData();
      showToast(`Added ₹${rechargeAmt} to ${cust.name}'s wallet!`);
    } catch (e: any) {
